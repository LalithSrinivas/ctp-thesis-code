...
def main(argv):
    ...
    for epoch_no in range(1, nb_epochs + 1):
        ...
        for batch_no, (batch_start, batch_end) in enumerate(batcher.batches, 1):
            ...
            end_batch = dt.datetime.now()
            if not is_quiet:
                logger.info(f'Epoch {epoch_no}/{nb_epochs}\tBatch {batch_no}/{nb_batches}\tLoss {loss_value:.6f}\t\
                                    Batch Time {end_batch-start_batch}')

        loss_mean, loss_std = np.mean(epoch_loss_values), np.std(epoch_loss_values)
        epoch_end = dt.datetime.now()
        logger.info(f'Epoch {epoch_no}/{nb_epochs}\tLoss {loss_mean:.4f} ± {loss_std:.4f}\tEpoch Time {epoch_end-epoch_start}')
