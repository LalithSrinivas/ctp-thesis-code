...
from ctp.evaluation import evaluate
from ctp.evaluation import evaluate_transe
...
def main(argv):
    ...
    parser.add_argument('--transe-eval', action='store_true', default=False)
    ...
    gntp_R = args.GNTP_R
    num_entities_select = args.top_k
    ...
    transe_eval = args.transe_eval

    evaluate_ = evaluate_transe if transe_eval else evaluate
    ...
    if 'countries' in train_path:
        ...
        for triples, name in [(t, n) for t, n in triples_name_pairs if len(t) > 0]:
            metrics = evaluate_(entity_embeddings=entity_embeddings, predicate_embeddings=predicate_embeddings,
                                ...
                                num_entities_select=num_entities_select)
